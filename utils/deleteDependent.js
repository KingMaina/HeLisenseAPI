...
let Detection = require('../model/detection');
let Motorbike = require('../model/motorbike');
...
const deleteDetection = async (filter) =>{
  try {
    let response  = await dbService.deleteMany(Detection,filter);
    return response;
  } catch (error){
...
const deleteMotorbike = async (filter) =>{
  try {
    let response  = await dbService.deleteMany(Motorbike,filter);
    return response;
  } catch (error){
    throw new Error(error.message);
  }
...
const deleteUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const detectionFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const detectionCnt = await dbService.deleteMany(Detection,detectionFilter);

      const motorbikeFilter = { $or: [{ vehicleOwner : { $in : user } },{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const motorbikeCnt = await dbService.deleteMany(Motorbike,motorbikeFilter);
      ...
      let response = {
        detection :detectionCnt,
        motorbike :motorbikeCnt,
...
const countDetection = async (filter) =>{
  try {
    const detectionCnt =  await dbService.count(Detection,filter);
    return { detection : detectionCnt };
  } catch (error){
    throw new Error(error.message);
  }
};

const countMotorbike = async (filter) =>{
  try {
    const motorbikeCnt =  await dbService.count(Motorbike,filter);
    return { motorbike : motorbikeCnt };
  } catch (error){
    throw new Error(error.message);
  }
};
...
const countUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const detectionFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const detectionCnt =  await dbService.count(Detection,detectionFilter);

      const motorbikeFilter = { $or: [{ vehicleOwner : { $in : user } },{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const motorbikeCnt =  await dbService.count(Motorbike,motorbikeFilter);
...
const softDeleteDetection = async (filter,updateBody) =>{  
  try {
    const detectionCnt =  await dbService.updateMany(Detection,filter);
    return { detection : detectionCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteMotorbike = async (filter,updateBody) =>{  
  try {
    const motorbikeCnt =  await dbService.updateMany(Motorbike,filter);
    return { motorbike : motorbikeCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteUser = async (filter,updateBody) =>{  
  try {
    ...
    if (user.length){
      ...
      const detectionFilter = { '$or': [{ addedBy : { '$in' : user } },{ updatedBy : { '$in' : user } }] };
      const detectionCnt = await dbService.updateMany(Detection,detectionFilter,updateBody);

      const motorbikeFilter = { '$or': [{ vehicleOwner : { '$in' : user } },{ addedBy : { '$in' : user } },{ updatedBy : { '$in' : user } }] };
      const motorbikeCnt = await dbService.updateMany(Motorbike,motorbikeFilter,updateBody);
